...
class morphoGrid:
    ...
    def loadHDF5(self, timestep=0, discharge2=None, sedload2=None):
        """
        ...
        #Matt Update
        iceTH = np.array((df['/iceTH']))

        x, y, z = np.hsplit(coords, 3)
        c = cumdiff

        if discharge2 is None:
            d = discharge
        else:
            d = discharge2
        ...
        #Matt Update
        ii[onIDs] = np.average(i_vals[onIDs, :], weights=(1. / distances[onIDs, :]), axis=1)
        ...
        self.iceTH = np.reshape(ii, (self.ny, self.nx))

        logdis = self.discharge.copy()
