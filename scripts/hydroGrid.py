...
class hydroGrid:
    ...
    def getCatchment(self, timestep=0):
        ...
        di = h5py.File('%s/tin.time%s.hdf5'%(self.folder, timestep), 'r')
        ...
        lsedload = np.array((df['/sedload']))
        
        vertices1 = np.array((di['/coords']))
        liceload1 = np.array((di['/iceTH']))
        _,id1 = spatial.KDTree(vertices1).query(vertices)
        liceload = liceload1[id1]
        ...
        Sedloadl = lsedload[lconnect[:,0]-1]
        IceTHl = liceload[lconnect[:, 0]-1]
        ...
        self.Sedload = Sedload[basinIDs]
        self.IceTH = IceTH[basinIDs]
    ...
    def extractMainstream(self):
        ...
        self.streamSedload = self.Sedload[streamIDs[id]]
        self.streamIceTH = self.IceTH[streamIDs[id]]

        return

    def computeParams(self, kd=None, kc=None, m=None, n=None, num=500):
        """
        Computes the Peclet and cumulative main stream length.
        ...
        self.streamSedload = self.streamSedload.reshape(len(self.streamSedload))
        self.streamIceTH = self.streamIceTH.reshape(len(self.streamChi))
        ...
        sedloadFunc = interpolate.interp1d(self.streamLght,
                                        self.streamSedload, kind='linear')
        IceTHFunc = interpolate.interp1d(self.streamLght,
                                        self.streamIceTH, kind='linear')
        ...
        self.Seddata = sedloadFunc(self.dist)
        self.IceData = IceTHFunc(self.dist)
    ...
    def computeTribParams(self, tribList=None, kd=None, kc=None, m=None, n=None, num=500):
        ...
        for t in range(len(tribList)):
            ...
            if len(trib.streamX) > 1:
                ...
                #Matt Update
                sedloadFunc = interpolate.interp1d(self.streamLght,
                                                trib.streamSedload, kind='linear')
                #Matt Update
                iceTHFunc = interpolate.interp1d(self.streamLght,
                                                trib.streamIceTH, kind='linear')
                ...
                trib.Seddata = sedloadFunc(trib.dist)
                trib.IceTH = iceTHFunc(trib.dist)
    ...
    def extractAllStreams(self, startIDs = None):
        ...
        for i in range(len(startIDs)):
            ...
            if append :
                ...
                trib.streamSedload = self.Sedload[stIDs]
    ...
    def limitAllstreams(self,ptXY,streamlist):
        ...
        for i in range(len(streamlist)):
            ...
            if len(found) > 0:
                ...
                #Matt Update
                ntrib.streamSedload = streamlist[i].streamSedload[selectIDs]
    ...
    def viewNetwork(self, markerPlot = True, linePlot = False, lineWidth = 3, markerSize = 15, \
        ...
        variable: val
            Name of the dataset to plot: 'chi', 'FA', 'Z', 'Sedload'(?)
        ...
        if val == 'chi':
            ...
            valData = self.Sedload
        else:
            #Matt Update
            raise RuntimeError('The requested data is unknown, options are: chi, Fa, Z. and maybe Sedload')
    ...
    def viewPlot(self, lineWidth = 3, markerSize = 5, xval = 'dist', yval = 'chi', \
        ...
        variable: xval
            Name of the dataset to plot along the X direction: 'dist', 'chi', 'FA', 'Z', 'Pe', 'Sedload'

        variable: yval
            Name of the dataset to plot along the Y direction: 'dist', 'chi', 'FA', 'Z', 'Pe', 'Sedload'
        ...
        if xval == 'dist':
            ...
            raise RuntimeError('The requested X value is unknown, options are: dist, chi, Fa, Z, Pe, Sedload')
        ...
        if yval == 'dist':
            ...
            raise RuntimeError('The requested Y value is unknown, options are: dist, chi, Fa, Z, Pe, Sedload')
    ...
    def GlacProfiles(self, pData=None, pDist=None, pIce=None, width=800, height=400, linesize=2, title='Profile evolution with time'):
        """
        Plot profile mean, max and min. Include ice thickness profile with filled area.

        Parameters
        ----------
        pData: list of lists or arrays
            Dataset to plot along Y axis representing elevation profiles.

        pDist: list of lists or arrays
            Dataset to plot along X axis representing distances.

        pIce: list of lists or arrays
            Ice thickness data to be added to pData for the secondary plot.
        ...
        Returns:
        -------
        Plots the provided profiles and fills the area representing ice thickness.
        """
        ...
        for i in range(len(pData)):
            # Elevation profile
            trace_elevation = go.Scatter(
                x=pDist[i],
                y=pData[i],
                mode='lines',
                line=dict(shape='spline', width=linesize),
                name=f'Trace {i}'
            )

            data.append(trace_elevation)

            # Ice thickness adjusted profile
            ice_adjusted_y = pData[i] + pIce[i]
            non_zero_mask = pIce[i] != 0  # Check where ice thickness is non-zero

            if any(non_zero_mask):  # Only if there are non-zero values in pIce
                # Fill area between elevation and ice-adjusted elevation
                trace_fill = go.Scatter(
                    x=[pDist[i][j] for j in range(len(pDist[i]))],
                    y=[ice_adjusted_y[j] if non_zero_mask[j] else pData[i][j] for j in range(len(pData[i]))],
                    mode='lines',
                    line=dict(width=0),  # No line, only fill
                    fill='tonexty',  # Fill the area between this and the previous trace
                    fillcolor='rgba(10, 120, 255, 0.4)',  # Darker blue fill color for ice
                    name=f'Glacier Fill {i}',
                    showlegend=False
                )

                trace_ice = go.Scatter(
                    x=[pDist[i][j] for j in range(len(pDist[i])) if non_zero_mask[j]],  # Only plot where ice is non-zero
                    y=[ice_adjusted_y[j] for j in range(len(ice_adjusted_y)) if non_zero_mask[j]],  # Only plot where ice is non-zero
                    mode='lines',
                    line=dict(shape='spline', width=linesize, dash='dash'),
                    name=f'Glacier Ice Trace {i}'
                )

                # Append both the ice fill and the ice trace
                data.append(trace_fill)
                data.append(trace_ice)
